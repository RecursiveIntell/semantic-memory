...
async fn main() {
    ...
    println!("\nRunning test search...");
    let results = store.search("turbo-quant compression codec kv-cache", Some(5), None, None).await.expect("search");
    println!("Search returned {} results", results.len());
    for (i, r) in results.iter().enumerate() {
        println!("  [{}] vector_rank={:?} cos_sim={:?} content={:.60}", 
            i, r.vector_rank, r.cosine_similarity, r.content);
    }
    
    let stats = store.stats().await.expect("stats");
    println!("\nStats: facts={}, chunks={}, docs={}, db={:.1}MB", 
        stats.total_facts, stats.total_chunks, stats.total_documents,
